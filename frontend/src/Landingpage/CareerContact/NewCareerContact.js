...
const NewCareerContact = () => {
 const [formData, setFormData] = useState({
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!formData.username) newErrors.username = "Enter your name";
    if (!formData.email) newErrors.email = "Enter your email";
    if (!formData.phoneNumber) newErrors.phoneNumber = "Enter your phone";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post("http://localhost:5000/career-form", formToSend, {
